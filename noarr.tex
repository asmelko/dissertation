...
Equally as with layouts, a specific traversal order can be defined as an object (such as \texttt{tiles} on Line~$2$ in the previous code sample) and further reused in the different parts of the code. It has the same benefits as the layout reusability, such as locality of change and reusal.

But this feature further extends to a perhaps more powerful concept we call \emph{layout agnosticism} and \emph{traversal agnosticism}. As first class objects, the information about the specific traversal or layout can be extracted from a function and passed just as arguments. These arguments will then serve as interfaces for the user to specify the traversal or layout:
...
The function \mintinline{c++}{matmul} is now agnostic to the layout or traversal specified and, regardless the passed-in layout, it will result in the same operations, just in a different order. The function can be called repeatedly with different combinations of layout and traversal transformations, which can be used to (ideally automatically) search the transformation space and find the most optimal traversal.
