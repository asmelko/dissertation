...
\begin{defn}[Layout]
  Suppose a regular $n$-dimensional array $A$ with dimension lengths $d_1, \dots, d_n$ and an index space $\mathcal{I} = \{0,\dots,d_1\}\times \dots \times \{0,\dots,d_n\}$. We define a \emph{memory layout} $\mathbb{L}$ of $A$ as a bijection $\mathbb{L}: I \to \{0,\dots,d_1\cdot \dots \cdot d_n\}$. 
\end{defn}
