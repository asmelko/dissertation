...
These tools are very easy to use, and their ability to combine then to form complex loop transformations make them very expressive. Perhaps the biggest downsight is that they are very difficult to extend and for majority of them, a precompiler with an extra preprocessing step, or a custom compiler extension is needed.

On the other hand, the target problems for annotation-based loop transformation tools are somehow limited. Naturally, the best use of them is made, when there is already a baseline implementation and the optimization is achieved just by adding single line on top of the loop. However, when developing an optimized solution from the scratch and when the scale of the program passes a certain complexity threshold, optimizing just by annotations may become difficult. Pehaps a good case study is to compare paralelizing using OpenMP, an also compiler-based library, with pure C++ libraries, such as TBB. Case studies generally confirm, that ease of expressing parallelism in OpenMP is traded for a lower abstraction level and flexibility. Language-based approaches, such as TBB, can utilize object-oriented design, for more control, fostering of good programming style and higher abstraction level for newly developed parallel programs~\cite{kegel2009using,ajkunic2012comparison,refsnes2011comparison}.

\subsection{Noarr Traversals}

The similarities in memory layout and its traversal led to the development of Noarr Traversals. It is based on an idea, that each single loop, in some sequence of nested loops, represents an iteration over a single dimension of a data structure. Assuming perfectly nested loops, such traversal can be interpreted as a Noarr layout and be subjected to the same transformations as a memory layout.
...
The traverser extracts dimensions from the passed-in layouts, and yields and index space corresponding to the cartesian product of the extracted dimensions (after removing duplicates). Changing the traversal order is done by applying the proto-structures to the traverser. To reorder the loops to a more cache-friendly \mintinline{c++}{'i'}, \mintinline{c++}{'k'}, \mintinline{c++}{'j'} order, the traversal can be rewritten as follows:
\begin{minted}[fontsize=\footnotesize,breaklines,frame=leftline]{c++}
...
Further, Noarr is compatible with parallelism. Considering algorithms with master data structures, the parallelism is usually a question of in which nested loop are we aiming to parallelize. There can be caveats of course, such as data dependencies, but they are usually easy to spot. Noarr traversal transformations are general enough to allow custom parallelism. Nevertheless, Noarr supports parallelization wrappers around TBB and OpenMP, providing a proof of concept that the library can be extended to various parallelization backends. An example of dividing computation tasks of a Hadamard product over a threads of a single CUDA block is as follows:
