...
Generally, a programming model serves as a high-level abstraction of the hardware architecture, allowing programmers to write code that can run and scale well without the prior knowledge of the underlying hardware specifics. Depending on the programming framework or GPU vendor, the terminology within the model varies, but the core concepts remain the same. For simplicity, we will continue with the description in CUDA terms.

The main building block of GPU hardware architecture are \emph{Streaming Multiprocessors} (SMs). SM encouples hundreds of compute cores, a register file, L1 cache also called \emph{shared memory} and it is able to independently schedule threads. A GPU consists of multiple SMs, high end cards reaching up to hundred SMs. All SMs are connected via a high-speed interconnect to the GPU's memory, called \emph{global memory}. Figure~\ref{fig:sm} shows a diagram of a single streaming multiprocessor.

\begin{figure}[h]
	\centering
	\begin{subfigure}{.5\textwidth}
        \centering
        \includegraphics[width=0.9\textwidth]{img/cpu-gpu}
...
In practise, this aspect needs to be considered when choosing data structures layouts, such that the memory accesses are ordered in a serial fashion (such as using column-major matrix in matrix multiplication or avoiding linked-lists). 

\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{img/coal1}
    \caption{Coalesced memory accesses~\cite{site:cuda}.}
    \label{fig:coal}
\end{figure}

\subsubsection{CPU--GPU data transfers}

Transferring data between CPU and GPU memory spaces can pose a significant bottleneck when developing GPU programs. Usually, the interconnect which is used to transfer data between the CPU and the GPU has a much lower bandwidth than the GPU memory. Therefore, it is important to minimize the number of data transfers between the host and the device. 

In true data-parallel algorithms, these data transfers can be overlapped with computation, as depicted on Figure~\ref{fig:transfer}. Such overlapping is possible thanks to multiple execution queues on the GPU, which can be manually filled by using \emph{streams} abstraction.

\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{img/host-device-cpy.png}
    \caption{Time diagram of non-overlapped (top) and overlapped (bottom) transfer and computation~\cite{site:cuda}.}
    \label{fig:transfer}
\end{figure}
...
To keep the number of active threads high for the most amount of time, intra-warp branching should be avoided. This can be achieved by lifting the branching to inter-warp level, replacing branches with arithmetic operations if possible, or by unrolling loops.
