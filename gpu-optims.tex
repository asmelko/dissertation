...
Graphics Processing Units (GPUs) have evolved from specialized hardware designed solely for rendering graphics to powerful, general-purpose processors capable of handling complex computations. In the early years, 3D acceleration carts, as they were used to be called, were devoted to offloading the rendering tasks, such as drawing pixels, filling polygons with textures or computing geometry, from the CPU.

And because processing pixels is inherently parallel, GPU architectures were being developed with different paradigms than ordinary CPUs. GPU chip semantic is oriented on high performance achieved by immense parallelism, with a design which promotes its further scalability. Its die surface is largly occupied by compute cores with very little space dedicated to cache and control, which contradics the CPU design. Thus, GPUs can be much more powerful than CPUs if the solution is carefully designed for it.

The shift from specialized graphics processing to general-purpose computing was marked by the development of programming frameworks such as NVIDIA's CUDA (Compute Unified Device Architecture) and OpenCL (Open Computing Language) by Kronos group. As extensions to C/C++, these frameworks simplified the process of writing code for GPU and streamlined the adoption of GPU for general-purpose programming. CUDA, in particular, has become the de facto standard for GPU programming due to its ease of use, performance, and wide adoption in the scientific community.

\subsection{GPU Architecture and Programming Model}

Generally, the relation between GPU hardware architecture and its programming model is that the model serves as a high-level abstraction of the hardware, allowing to write a code that can run disregarding the underlying GPU. Depending on the programming framework or GPU vendor, the terminology within the model varies, but the core concepts remain the same. For simplicity, we will describe programming model in CUDA terms.

From hardware perspective, GPU cores are organized into \emph{Streaming Multiprocessors} (SMs). Each SM consists of compute units, a register file and L1 cache also called \emph{shared memory}. A GPU has multiple SMs (nowadays hundreds), each capable of executing tens of parallel threads. All SMs are connected via a high-speed interconnect to the GPU's memory, called \emph{global memory}.

The abstraction of this highly-parallel architecture is based on organizing threads into hierarchical levels. The basic unit of execution is a \emph{thread}, which is a single instance of the code running on the GPU. Threads are organized into \emph{blocks}, which are groups of threads that are executed on the same SM. Finally, a group of blocks, which are executed independently on multiple SMs, is called \emph{grid}. A grid completely defines the amount of concurrent work for a specified function to be executed on a GPU (also called a \emph{kernel}).

Conceptually, the GPU programming model is based on \emph{Single Instruction, Multiple Data} (SIMD) architecture, where a single instruction is executed on multiple data streams. In practise, a SM issues threads in groups of 32, \emph{warps}, which execute in \emph{lockstep} --- performing the same instruction at once. Other sources refer to this as \emph{Single Instruction, Multiple Threads} (SIMT) architecture, which is a more accurate description of the GPU execution model.

Naturally, such programming model favors data-parallel problems, consisting of the same independent computation on a large dataset. 

\subsection{Optimizing GPU performance}

Thankfully, contemporary GPU devices are becoming increasingly sophisticated and are equipped with variety of control-flow mechanisms, interthread communication, atomic instruction and others. This allows for a wider range of problems to be solved on GPUs. However, applying these features correctly is not trivial and requires a deep understanding of the GPU architecture and programming model. Thus, achieving optimal performance on a GPU requires a careful consideration of various aspects. We detail the most important problems to put in the consideration when optimizing GPU code.

\subsubsection{Arithmetic intensity}

The achievable performance of contemporary high-end GPUs is nowadays counted in teraflops (trillions of floating-point operations per second). However, to determine the actual performance, the memory badwidth needs to be consideres, which, simply put, can not feed the cores with data at the same rate as they can compute. A helpful metric to consider here is \emph{ops-per-byte} ratio, which is computed as the ratio of math and memory bandwidth and determines how many math operations need to be executed per transfered byte to achieve the maximum performance. For example, NVIDIA V100 GPU has a peak single-precision floating point bandwidth of $14$ teraflops while memory bandwidth only $900$GB/s. This results in ops-per-byte ratio of $\frac{14}{0.9} \approx 15$. Therefore, a kernel that performs less than $60$ single-precision floating point operations per $4$-byte load/store will be memory bound. This ratio of math operations to memory operations is called \emph{arithmetic intensity} and is a key factor in determining the performance of a GPU kernel.
...
Another GPU aspect, which is directly linked to arithmetic intensity is the memory hierarchy. The global off-chip memory has the highest latency and the lowest bandwidth. Fetching data from global memory takes hundreds of cycles, whereas on-chip shared memory can be accessed a magnitute faster. In an extreme case, an optimized program can utilize a rather big register file as a cache, with virtually zero latency. The key to achieving high performance on a GPU is to minimize global memory accesses by smartly dividing the work such that threads in a block can utilize shared memory.

Also, since ops-per-byte ratio is inversly proportional to the memory bandwidth, the arithmetic intensitiy of a program using a shared memory does not have to be that high to achieve the maximum performance.

\subsubsection{Memory coalescing}

When accessing global memory is unavoidable, it is important to consider the memory coalescing to not further hinder the memory bandwidth. GPUs service memory requests in $128$B wide transactions of continuous data. In order to achieve the maximum bandwidth, the memory accesses of threads should minimize the number of transactions they generate. This can be achieved when accesses of threads in a warp coalesce to access aligned continuous memory locations.

Accesses to data structures need to be ordered in a serial fashion.
row-col matrix, linked-list

\subsubsection{Host-device transfers}

The data transfers between the host and the device are a significant bottleneck in GPU programming. The PCIe bus, which is used to transfer data between the CPU and the GPU, has a much lower bandwidth than the GPU's memory. Therefore, it is important to minimize the number of data transfers between the host and the device. 
In true data-parallel algorithms, this is not a problem. Can be perfectly overlapped. This can be achieved by overlapping the data transfers with computation, using pinned memory, or by using the \emph{Unified Memory} feature of CUDA, which allows the GPU to access the CPU's memory directly.

\subsubsection{Thread divergence}

One downside of the highly-concurrent hardware architecture is that diverging execution paths are punished by serialization. It is important to avoid branching of threads within a warp as much as possible. Branches can be mitigated by lifting them to block or grid level, where the diverging threads can be executed in parallel. Sometimes, branches can be replaced by equivalent arithmetic operations. Loop unrolling.

\subsubsection{Thread occupancy}

The GPU is designed to run thousands of threads in parallel. However, the number of threads that can be executed concurrently is limited by the number of available resources on the SM. The occupancy of a kernel is the ratio of the number of active warps to the maximum number of warps that can be executed concurrently. The occupancy can be increased by reducing the number of registers used by a thread, increasing the number of threads per block, or reducing the amount of shared memory used by a block.

Want many active warps to hide latencies. But if have many, have small thread efficiency - not utilizing faster caches, using less registers. 
