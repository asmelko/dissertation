...
Another useful metric, especially for GPUs, is to obtain the \emph{ops per byte} ratio. It is computed as the GPU's math bandwidth divided by the memory bandwidth and represents the maximum number of operations that can be performed on a single byte of data. To put it into a perspective, ops per byte ratio of a modern NVIDIA V100 GPU is 
