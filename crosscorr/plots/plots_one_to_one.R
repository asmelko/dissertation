...
{
    data = read.csv('../data/ampere/one_to_one_saturated/multirow_both.csv', header=T, sep=',')
...
{
    data = read.csv('../data/ampere/one_to_one/work_distribution.csv', header=T, sep=',')
    data = data.frame(data %>% group_by_at(names(data)[-grep("(Kernel)|(Kernel_iterations)|(X)|(Args)", names(data))]) %>% summarise(Kernel = mean(Kernel)))
    data = subset(data, distribution_type == "triangle")
    data = subset(data, warps_per_thread_block == 4)

    data_basic = read.csv('../data/ampere/one_to_one/basic.csv', header=T, sep=',')
...
{
    data = read.csv('../data/ampere/one_to_one/warp-per-shift-shared-memory.csv', header=T, sep=',')
    data = data.frame(data %>% group_by_at(names(data)[-grep("(Kernel)|(Kernel_iterations)|(X)|(Args)", names(data))]) %>% summarise(Kernel = mean(Kernel)))
    ...
    ggsave("one-to-one/warp-per-overlap-shared-memory.pdf", device='pdf', units="in", scale=S, width=W, height=H*1.5,
...
{
    data = read.csv('../data/ampere/one_to_one/warp-per-shift.csv', header=T, sep=',')
    data = data.frame(data %>% group_by_at(names(data)[-grep("(Kernel)|(Kernel_iterations)|(X)|(Args)", names(data))]) %>% summarise(Kernel = mean(Kernel)))
    
    ggsave("one-to-one/warp-per-overlap.pdf", device='pdf', units="in", scale=S, width=W, height=H,
...

{
    ...
    data_wd = read.csv('../data/ampere/one_to_one/work_distribution.csv', header=T, sep=',')
    ...
    data_wps = read.csv('../data/ampere/one_to_one/warp-per-shift.csv', header=T, sep=',')
    data_wps["alg"] = "warp-per-overlap"
    data_wps = subset(data_wps, shifts_per_thread_block == 4)
    data_wps = subset(data_wps, select=c(alg, Kernel, Input.matrix.rows))

    data_wpsi = read.csv('../data/ampere/one_to_one/warp-per-shift-simple-indexing.csv', header=T, sep=',')
    data_wpsi["alg"] = "warp-per-overlap-simple-idx"
    data_wpsi = subset(data_wpsi, shifts_per_thread_block == 4)
    data_wpsi = subset(data_wpsi, select=c(alg, Kernel, Input.matrix.rows))

    data_wpss = read.csv('../data/ampere/one_to_one/warp-per-shift-shared-memory.csv', header=T, sep=',')
    data_wpss["alg"] = "warp-per-overlap-shared"
    ...
    ggsave("one-to-one/one-to-one-warp-per-overlap-and-split-row.pdf", device='pdf', units="in", scale=S, width=W, height=H,
...

{
    ...
    data_original = read.csv('../data/ampere/one_to_one/original.csv', header=T, sep=',')
    data_original["alg"] = "baseline"
    ...
    data_mb = read.csv('../data/ampere/one_to_one/multirow_both.csv', header=T, sep=',')
    ...
    data_fft = read.csv('../data/ampere/one_to_one/fft.csv', header=T, sep=',')
    data_fft["alg"] = "fft"
    ...
    data_fft2 = read.csv('../data/ampere/one_to_one/fft.csv', header=T, sep=',')
    ...
    data_original["cmp"] = "baseline comparison"
    data_wd["cmp"] = "baseline comparison"
    data_mb["cmp"] = "baseline comparison"
    data_wps["cmp"] = "baseline comparison"
    ...
    data$cmp = factor(data$cmp, levels=c("baseline comparison", "fft comparison"))
