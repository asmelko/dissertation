...
{
    data = read.csv('../data/ampere/n_to_mn_saturated/multimat-right.csv', header=T, sep=',')
...
{
    data = read.csv('../data/ampere/n_to_mn_saturated/multimat-right-multirow-both.csv', header=T, sep=',')
    data = data.frame(data %>% group_by_at(names(data)[-grep("(Kernel)|(Kernel_iterations)|(X)|(Args)", names(data))]) %>% summarise(Kernel = mean(Kernel)))
...
{
    data = read.csv('../data/ampere/n_to_mn/multimat-right-multirow-both.csv', header=T, sep=',')
    data = data.frame(data %>% group_by_at(names(data)[-grep("(Kernel)|(Kernel_iterations)|(X)|(Args)", names(data))]) %>% summarise(Kernel = mean(Kernel)))
...
{
    data = read.csv('../data/ampere/n_to_mn/multimat-right-work-distribution.csv', header=T, sep=',')
    data = data.frame(data %>% group_by_at(names(data)[-grep("(Kernel)|(Kernel_iterations)|(X)|(Args)", names(data))]) %>% summarise(Kernel = mean(Kernel)))

    data = subset(data, distribution_type == "triangle")

    data_basic = read.csv('../data/ampere/n_to_mn/multimat-right.csv', header=T, sep=',')
...

{
    ...
    data_original = read.csv('../data/ampere/n_to_mn/original.csv', header=T, sep=',')
    data_original["Algorithm"] = "baseline"
    data_original = subset(data_original, select=c(Algorithm, Kernel, Input.matrix.rows, Input.left.matrices, Input.right.matrices))

    data_wd = read.csv('../data/ampere/n_to_mn/multimat-right-work-distribution.csv', header=T, sep=',')
    ...
    data_mb = read.csv('../data/ampere/n_to_mn/multimat-right-multirow-both.csv', header=T, sep=',')
    ...
    data_fft = read.csv('../data/ampere/n_to_mn/fft.csv', header=T, sep=',')
    ...
    data_fft2 = read.csv('../data/ampere/n_to_mn/fft.csv', header=T, sep=',')
...
#     data_wd = read.csv('../data/ampere/one_to_many/multimat-right-work-distribution.csv', header=T, sep=',')
#     data_wd["Algorithm"] = "split-row"
#     data_wd = subset(data_wd, warps_per_thread_block == 4)
#     data_wd = subset(data_wd, distribution_type == "triangle")
#     data_wd = subset(data_wd, rows_per_thread == 1)
...
#     data_mb = read.csv('../data/ampere/one_to_many/multimat-right-multirow-both.csv', header=T, sep=',')
#     data_mb["Algorithm"] = "grouped-overlap"
#     data_mb = subset(data_mb, warps_per_thread_block == 4)
#     data_mb = subset(data_mb, left_rows_per_iteration == 2)
#     data_mb = subset(data_mb, shifts_per_thread_right_matrix == 2)
...

{
    ...
    ggsave("n-to-mn/n-to-mn-one-to-many-streams.pdf", device='pdf', units="in", scale=S, width=W, height=H,
        ggplot(data, aes(x=Input.matrix.rows,y=Kernel, color=type, shape=type)) +
        geom_point(size=point_size) +
        geom_line(linewidth=line_size) +
        xlab("Matrices size (log-scale)")+
